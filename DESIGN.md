...
## 8. Hero WebGL de /soluciones — excepción documentada a las reglas de motion (2026-08-12)

`src/components/ui/horizon-hero-section.tsx` (`HorizonHeroSection`), montado como header de `/soluciones` en reemplazo del bloque de texto plano anterior. Es una **excepción explícita, solicitada y confirmada por el cliente** a las reglas de motion de este sistema — no un precedente para usarlo en otra parte del sitio sin la misma confirmación.

- **Qué rompe a propósito:** DESIGN.md (Sección 6, Don'ts) prohíbe "motion decorativo sin propósito" y dice que `Reveal` (fade-in on scroll) es el único patrón de entrada permitido, "sin parallax, sin efectos de tipo máquina de escribir, sin loops". Este componente es una escena WebGL (Three.js) con partículas, bruma animada y bloom — el opuesto del sistema plano/documental del resto del sitio. Se presentaron estas reglas al cliente explícitamente antes de implementar; confirmó proceder de todas formas, exclusivamente para este hero.
- **Qué NO rompe:** la paleta. Toda la escena usa únicamente los 3 colores de marca (`#D6532B` terracota, `#53843A` oliva, `#FBE369` amarillo) + neutros de tinta — cero azul/morado/dorado genérico de las plantillas de las que viene este patrón. El copy es real (reutiliza terminología ya validada: HPLC, GC, IQ/OQ/PQ, los 6 sectores), no las frases de placeholder del componente original ("HORIZON/COSMOS/INFINITY").
- **Simplificado respecto al componente original:** el snippet original usaba una sección `300vh` con `position: sticky` y la cámara viajando entre 3 "momentos" narrativos disparados por scroll (GSAP + cálculo manual de progreso). Se detectó con Playwright que esa coreografía producía un frame completamente negro/vacío a mitad de scroll (conflicto entre las actualizaciones de `visibility` de GSAP y los re-renders de React disparados por el estado de scroll, más una trayectoria de cámara que podía terminar sin nada visible en pantalla). Se simplificó a una escena ambiental de una sola altura de viewport (`h-dvh`, sin scroll-jacking): cámara fija con flotación sutil, un solo título/subtítulo, sin narrativa de 3 actos. Verificado sin errores de consola ni de página en desktop y mobile.
- **Dependencias nuevas:** `three` + `@types/three` + `gsap`. `gsap` no reemplaza a `motion/react` (la librería de animación del resto del sitio) — su uso queda acotado a la animación de entrada de este hero puntual.
- **Accesibilidad:** respeta `prefers-reduced-motion` (fondo estático con gradiente en vez de WebGL/GSAP cuando el usuario lo prefiere).
