...
- **Mega-menú animado (solo Productos, agregado 2026-08-12):** El trigger "Productos" del nav de escritorio (`src/components/sections/navigation.tsx`) usa `MotionNavigationMenu` (`src/components/ui/motion-navigation-menu.tsx`, adaptado a los tokens del proyecto — no shadcn genérico) con un fondo animado que sigue al item con hover (`src/components/ui/highlight.tsx`, primitivo propio vía `motion/react` con `layoutId`) y un panel de viewport con spring physics. Contenido real: 6 categorías de producto curadas de `mock-products.ts`, todas hacia `/productos` (no existe deep-link por categoría). El trigger es un `<button>`, no un `<a>` — el panel incluye "Ver catálogo completo" como única vía de navegación directa, y el contenido se registra en cliente (`useLayoutEffect`), por lo que no está en el HTML servido. Mitigado porque `/productos` ya está enlazado desde el footer y `sitemap.xml`.
- **Dropdown "Servicios" (corregido 2026-08-12, ya NO usa `MotionNavigationMenu`):** `src/components/sections/services-nav-dropdown.tsx`, componente autocontenido, sin el sistema de highlight/viewport compartido — a propósito, porque el patrón pedido es distinto: "Servicios" debe seguir siendo un `<a href="/servicios">` real y navegable, con una flecha independiente (`<button aria-expanded aria-controls>`) que abre/cierra el panel. Soporta hover en escritorio (solo `pointerType === "mouse"`, no se activa por touch), click/tap, teclado (Enter/Espacio nativos del `<button>`, Escape cierra y devuelve foco, click fuera cierra, cambio de ruta cierra). Zonas táctiles ≥44×44px. Panel siempre presente en el DOM (visibility/opacity, no `display:none`) para que los 4 links sean crawleables sin JS; toggle vía `invisible`/`visible` + `opacity`/`translate-y` (máx. 4px), 150ms ease-out, sin spring/bounce/scale, `motion-reduce:transition-none`. Variante `mobile` se integra en el drawer existente (accordion inline, sin posición absoluta). Contenido: los 4 servicios reales confirmados (ver más abajo), enlazando a anclas `/servicios#{id}` — los cards de `/servicios/page.tsx` ahora tienen `id={service.id}` + `scroll-mt-32/40` para que el anchor no quede tapado por el header fijo.
  - **Taxonomía de servicios — inconsistencia detectada, no resuelta todavía:** `src/content/site.ts` (`services`) tiene 4 servicios distintos ("Implementación HPLC", "Métodos analíticos por GC", "Validación y trazabilidad", "Mantención y soporte técnico") que alimentan las páginas huérfanas `/servicios/[slug]` — huérfanas porque ningún link del sitio apunta a ellas. La página real `/servicios` (cards visibles) y el flujo completo `/contacto/[tipo]` (con `generateStaticParams`) coinciden en una taxonomía DISTINTA y consistente entre sí: **Mantención, Correctivo, Diagnóstico, Capacitación**. El dropdown de Servicios usa esta segunda taxonomía (la real/visible), no `site.ts`. Pendiente: decidir si `site.ts` y `/servicios/[slug]` se actualizan para coincidir, o se eliminan — no se tocó en esta sesión por estar fuera del alcance pedido (solo el dropdown).
