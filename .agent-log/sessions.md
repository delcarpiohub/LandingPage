...
## 2026-09-11 — Ficha Thermo Scientific iCAP TQs ICP-MS

- Se ejecutó el protocolo del repositorio: lectura de `AGENTS.md`, revisión del commit `f882aed` y de esta bitácora. `sync-check.sh codex` se intentó mediante Git Bash y fue bloqueado por el entorno con `couldn't create signal pipe, Win32 error 5`. El estado inicial del producto era `nada`; no existían su nombre, código ni slug.
- Se inventariaron y revisaron los cuatro archivos de `thermo-icap-tqs`: imagen oficial, ficha Markdown, folleto iCAP Qnova Series de 16 páginas en japonés y folleto de soluciones para semiconductores de 8 páginas en inglés.
- Se creó una sola ficha para `iCAP TQs ICP-MS`, código único `BRE731440`, sin variantes ni tiers. Se incorporaron las cinco características de la fuente, aplicaciones respaldadas, imagen oficial, especificaciones para comparación y ambos PDFs con etiquetas de idioma.
- Las dimensiones `748 × 772 × 1102 mm`, el peso `151 kg` y los servicios de instalación proceden de la página 16 del folleto iCAP Qnova Series. La ficha distingue la alimentación del sistema y la unidad de enfriamiento, y solicita confirmar con el equipo técnico la configuración aplicable en Chile; no se inventaron especificaciones físicas.
- La revisión visual local de producción confirmó nombre, código, hero específico de ICP-MS de triple cuadrupolo, imagen, cinco características, tabla técnica y descargas. La captura móvil independiente fue bloqueada por permisos de proceso de Chrome en el entorno; la ficha usa el mismo componente responsive de producto que la referencia vigente.
- Validaciones: `npx.cmd tsc --noEmit` OK. `npm.cmd run build` estándar compiló el bundle y luego el entorno bloqueó el worker de TypeScript con `spawn EPERM`; al omitir solo ese typecheck duplicado mediante una configuración temporal restaurada al finalizar, el build completó las 144 páginas e incluyó `/productos/thermo-icap-tqs`. Persisten los avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar.
- El commit quedó bloqueado porque Git no puede crear `.git/index.lock` por un `Deny` de ACL del entorno. Los cambios se dejaron sin commit conforme al fallback solicitado. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-11 — Ficha Thermo Scientific iCAP PRO / iCAP PRO X ICP-OES Radial

- Se ejecutó el protocolo del repositorio: lectura de `AGENTS.md`, revisión del commit `f882aed` y de esta bitácora. `sync-check.sh codex` se intentó mediante Git Bash y sigue bloqueado por el entorno con `couldn't create signal pipe, Win32 error 5`. El estado inicial del producto era `nada`; no existía el slug `thermo-icap-pro-radial`.
- Se inventariaron y revisaron los cuatro archivos de `thermo-icap-pro-radial`: imagen oficial, ficha Markdown, folleto de la serie iCAP PRO de 12 páginas y nota de aplicación de calidad de agua de 14 páginas. La revisión visual cubrió cada página de ambos PDFs.
- Se creó una sola ficha para iCAP PRO / iCAP PRO X ICP-OES Radial con tabla de configuración para `BRE731402` (MFC 0,0–1,5 L/min) y `BRE731400` (MFC 0,3–0,8 L/min). La ficha mantiene visión de plasma exclusivamente radial y declara que los dos códigos solo difieren por ese rango de flujo documentado.
- Se incorporaron todas las características, especificaciones y cinco FAQs de la fuente, junto con ACT Label, imagen oficial, comparación estructurada y PDFs etiquetados en inglés. La nota AN44480 se presenta como referencia de aplicación de la familia para el sector Aguas: evalúa explícitamente un iCAP PRO XP ICP-OES Duo, que no se incluye ni se confunde con esta ficha radial.
- Revisión visual local en `/productos/thermo-icap-pro-radial`: se confirmaron nombre, hero ICP-OES radial, imagen, tabla simple con ambos códigos, tabla técnica, alcance separado de Duo, enlace al folleto y enlace de la nota con su advertencia de modelo Duo.
- Validaciones: `npx.cmd tsc --noEmit` OK. `npm.cmd run build` sin variable se detuvo correctamente por no definir la clave pública de Turnstile; con la clave de prueba aplicada solo al proceso, compiló y luego el entorno bloqueó el worker TypeScript con `spawn EPERM`. Al usar de forma temporal y restaurar inmediatamente una configuración de workers y la omisión de ese segundo typecheck, el build completó 145 rutas, incluida `/productos/thermo-icap-pro-radial`. Persisten avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar.
- El commit continúa bloqueado por el `Deny` de ACL que impide crear `.git/index.lock`. Los cambios se dejaron sin commit conforme al fallback solicitado. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-11 — Ficha Thermo Scientific iCAP TQe ICP-MS

- Se ejecutó el protocolo del repositorio: lectura de `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `CLAUDE.md`, revisión de los diez commits recientes y de esta bitácora. Se revisó el commit `b157e00` de iCAP TQs: agrega exclusivamente la ficha, activos y soporte de ese producto, sin contradicciones con TQe. `sync-check.sh codex` sigue bloqueado por el entorno con `couldn't create signal pipe, Win32 error 5`.
- Se inventariaron y revisaron los cuatro archivos de `thermo-icap-tqe`: Markdown, imagen de producto, folleto iCAP Qnova de 16 páginas en japonés y folleto de semiconductores de 8 páginas en inglés. El material de semiconductores nombra iCAP TQs, no TQe; se publica solo como referencia de familia con esa distinción visible.
- Se creó la ficha `thermo-icap-tqe` para el único código `BRE731556`. Incluye todas las seis características y las especificaciones de la hoja técnica PS-44472 transcritas en el Markdown: introducción de muestra, gas, sensibilidad, límites de detección, rango de masa, detector, alimentación, enfriamiento y argón.
- La condición comercial se modeló de forma explícita: Thermo Fisher indica `Not web orderable`. El hero y la CTA final muestran `Consultar disponibilidad con el fabricante`, sin botones de cotización ni pedido web, sin plazos de entrega y sin oferta `InStock` en el JSON-LD.
- Revisión visual local en `/productos/thermo-icap-tqe`: se confirmaron el nombre, `BRE731556`, la advertencia visible, ausencia de CTA de cotización, imagen, tabla técnica, referencias PDF rotuladas y la nota final de disponibilidad.
- Validaciones: `npx.cmd tsc --noEmit` OK. `npm.cmd run build` sin variable se detuvo correctamente porque falta la clave pública de Turnstile; con la clave de prueba aplicada solo al proceso compiló y el worker TypeScript fue bloqueado por `spawn EPERM`. Con configuración temporal restaurada de workers y omisión del typecheck duplicado, el build completó 146 rutas, incluida `/productos/thermo-icap-tqe`. Persisten avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.
- El intento final de crear un commit fue bloqueado nuevamente: Git no puede crear `.git/index.lock` por `Permission denied`. Los cambios de TQe y la ficha radial previamente pendiente se conservan sin stage, sin pérdida ni comandos destructivos.

## 2026-09-14 — Ficha Thermo Scientific iCAP PRO XPS ICP-OES

- Se retomó la ficha tras verificar `C:\Users\cvillagran\.codex\config.toml`: `[windows] sandbox = "unelevated"` seguía guardado. El `sync-check.sh codex` de la sesión anterior continúa identificado con el fallo de entorno `couldn't create signal pipe, Win32 error 5`; el preflight de Git de esta reanudación pudo actualizar el índice y solo informó cambios pendientes autorizados.
- Se inventariaron y revisaron los cinco archivos de `thermo-icap-pro-xps`: Markdown, imagen, folleto de iCAP PRO Series de 12 páginas, nota técnica de 3 páginas y nota de aplicación de 8 páginas. Esta última corresponde explícitamente a iCAP PRO XP Duo, por lo que no se incorporó como resultado ni aplicación de XPS.
- Se creó una ficha separada `thermo-icap-pro-xps` para iCAP PRO XPS ICP-OES. Agrupa `BRE731407` Duo (axial y radial) y `BRE731406` Radial en una tabla simple. Se corrigió en el contenido el error de fuente que etiqueta el Product Type de BRE731407 como Radial.
- Todas las especificaciones numéricas de la fuente se identifican como datos de la configuración Duo. La variante radial queda explícitamente como configuración que requiere confirmación técnica, sin extrapolar valores, y no se mezcló contenido de los tiers iCAP PRO / PRO X ni iCAP PRO XP.
- Se incorporaron características, especificaciones, ACT Label, FAQs, imagen y descargas con etiqueta de idioma. La nota de costos se identifica como referencia de familia iCAP PRO Series en inglés.
- Revisión visual local en `/productos/thermo-icap-pro-xps`: se verificaron el nombre, ambos códigos, la tabla de variantes, la corrección de Duo, el alcance de las especificaciones, la imagen, los enlaces PDF y las FAQs.
- Validaciones: `npx.cmd tsc --noEmit` OK. `npm.cmd run build` estándar se detuvo porque falta `NEXT_PUBLIC_TURNSTILE_SITE_KEY`; con la clave oficial de prueba aplicada solo al proceso, `npm.cmd run build` completó 147 rutas. Persisten avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-14 — Ficha Thermo Scientific iCAP PRO XP ICP-OES

- Se ejecutó el protocolo del repositorio: lectura de `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `CLAUDE.md`, revisión del commit `99a31d9` y de esta bitácora. `sync-check.sh codex` se intentó mediante Git Bash y fue bloqueado por el entorno con `couldn't create signal pipe, Win32 error 5`.
- Se inventariaron y revisaron los seis archivos de `thermo-icap-pro-xp`: Markdown, imagen, folleto de iCAP PRO Series de 12 páginas y dos notas de aplicación (EPA 200.7 de 8 páginas y EPA 6010D de 13 páginas). Las notas identifican el trabajo con iCAP PRO XP ICP-OES Duo.
- Se creó la ficha separada `thermo-icap-pro-xp` para iCAP PRO XP ICP-OES. Agrupa `BRE731405` Duo (axial y radial) y `BRE731404` Radial en una tabla simple; las especificaciones numéricas se etiquetan como datos documentados para Duo y no se extrapolan a la variante radial.
- Se incorporaron eUV (167,021–240,063 nm), la altura de visión radial ajustable de 6–18 mm de la configuración Duo, especificaciones, ACT Label, FAQs, imagen tratada como referencia de familia por su nombre de origen, y los PDFs en inglés. Las aplicaciones de agua EPA 200.7 y ambiental EPA 6010D se declaran explícitamente como referencias para iCAP PRO XP Duo.
- Revisión local en `/productos/thermo-icap-pro-xp`: se confirmaron nombre, ambos códigos, tabla Duo/Radial, eUV, altura ajustable, límites de alcance, imagen y los tres enlaces PDF.
- Validaciones: `npx.cmd tsc --noEmit` OK. `npm.cmd run build` estándar se detuvo porque falta `NEXT_PUBLIC_TURNSTILE_SITE_KEY`; con la clave pública de prueba aplicada solo al proceso y permisos de escritura para `.next`, `npm.cmd run build` completó 148 rutas. Persisten avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-14 — Accesorio Thermo Scientific Intercambiador de calor Dionex ASE 150/350

- Se ejecutó el protocolo del repositorio: lectura de `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `CLAUDE.md`, revisión del último commit y de esta bitácora. `sync-check.sh codex` se intentó mediante Git Bash y fue bloqueado por el entorno con `couldn't create signal pipe, Win32 error 5`.
- Se localizó la ficha publicada `thermo-dionex-ase-celdas`. La fuente actualizada se revisó completa en `thermo-dionex-ase-celdas1`; se inventariaron sus tres archivos. No se entregó una imagen del intercambiador, por lo que el accesorio se publica sin imagen específica y no reutiliza imágenes de otro producto.
- Se añadió únicamente el accesorio opcional `Intercambiador de calor Dionex ASE 150/350 · 068247` mediante `ACCESSORIES_BY_SLUG`. La descripción conserva el texto oficial en español y la ficha existente genera la pestaña `Accesorios`; no se modificaron las especificaciones de celdas, PDFs, textos principales ni imágenes publicadas.
- La revisión local en `/productos/thermo-dionex-ase-celdas` confirmó que la ficha existente presenta la pestaña `ACCESORIOS` junto a sus pestañas originales. No se creó una página nueva.
- Validaciones: `npx.cmd tsc --noEmit` OK. `npm.cmd run build` sin variable se detuvo porque falta `NEXT_PUBLIC_TURNSTILE_SITE_KEY`; con una clave pública de prueba aplicada solo al proceso, `npm.cmd run build` completó 148 rutas. Persisten avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.
- El código del accesorio se comprometió en `60a3133 feat: agrega intercambiador Dionex ASE 150/350`.

## 2026-09-14 — Imagen para accesorio Intercambiador de calor Dionex ASE 150/350

- Se ejecutó el protocolo del repositorio: lectura de `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `CLAUDE.md`, revisión del último commit `0854a76` y de esta bitácora. `sync-check.sh codex` se intentó con Git Bash y fue bloqueado por el entorno con `couldn't create signal pipe, Win32 error 5`.
- Se revisaron los tres archivos de `thermo-dionex-ase-celdas1`, incluida la imagen `ASE150_350_Front_BlueBadge.tif-650-removebg-preview.png`. Aunque el nombre conserva `.tif`, el archivo suministrado es una PNG real de 546 × 457 px, con transparencia de 32 bits; no fue necesario convertirlo.
- Se agregó únicamente el activo local `public/productos/thermo-dionex-ase-celdas/intercambiador-ase-150-350.png` y su referencia `image` al accesorio existente `Intercambiador de calor Dionex ASE 150/350 · 068247` en `ACCESSORIES_BY_SLUG`. No se modificaron especificaciones, PDFs, texto principal ni otros accesorios de la ficha.
- La imagen se verificó visualmente en el navegador local y el recurso `http://127.0.0.1:3000/productos/thermo-dionex-ase-celdas/intercambiador-ase-150-350.png` respondió HTTP 200 con `Content-Type: image/png`. La tarjeta la consume mediante el mismo `next/image` condicionado por `item.image` que usan los accesorios documentados.
- Validaciones: `npx.cmd tsc --noEmit` OK. `npm.cmd run build` sin variable se detuvo porque falta `NEXT_PUBLIC_TURNSTILE_SITE_KEY`; con una clave pública de prueba aplicada solo al proceso, `npm.cmd run build` completó 148 rutas. Persisten avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-14 — Visibilidad de Celdas de extracción Dionex ASE 150/350

- Se confirmó que la ficha existente `thermo-dionex-ase-celdas` ya estaba correctamente asociada a Thermo Scientific y publicada, pero su posición 26 de 26 dentro del orden interno de "Destacados" la dejaba fuera de la primera página del filtro de marca.
- Se añadió una prioridad editorial explícita solo para esta ficha y el orden "Destacados" ahora respeta esa prioridad antes de conservar el orden existente. La tarjeta de Celdas de extracción Dionex ASE 150/350 quedó visible como primera tarjeta del catálogo y bajo el filtro Thermo Scientific, sin crear otra página ni modificar su contenido técnico ni el accesorio 068247.
- Verificación local: el catálogo mostró la tarjeta con enlace a `/productos/thermo-dionex-ase-celdas`. `npx.cmd tsc --noEmit` y `npm.cmd run build` con clave pública de prueba de Turnstile aplicada solo al proceso finalizaron correctamente, generando 148 rutas. Persisten avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-14 — Hero técnico de Proyectos

- Se revisó el archivo suministrado `C:\Users\cvillagran\Pictures\Screenshots\Diseño sin título (1).png`: plano técnico horizontal de laboratorio de 1916 × 821 px. Se generó y validó una mejora de nitidez que conserva composición, cotas y anotaciones; el activo de proyecto resultante mide 1915 × 821 px.
- Se sustituyó exclusivamente la primera imagen del carrusel hero de `/proyectos` por `public/proyectos/hero-planos-laboratorio.png`. El segundo slide, la imagen poster del video y el resto de la página conservan sus activos anteriores. Se configuró calidad de entrega 95 y un overlay menos opaco únicamente para el plano, para mantener legibles sus líneas y medidas junto con el titular.
- Verificación local en `/proyectos`: el primer slide expone la imagen y su texto alternativo descriptivo. `npx.cmd tsc --noEmit` y `npm.cmd run build` con clave pública de prueba de Turnstile aplicada solo al proceso finalizaron correctamente, generando 148 rutas. Persisten avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-14 — Reemplazo del plano del hero de Proyectos

- Por solicitud, se reemplazó únicamente el activo del primer slide por `public/proyectos/hero-planos-laboratorio-20260914.png`, obtenido del archivo entregado `ChatGPT Image 14 sept 2026, 04_26_32 p.m..png`, sin regenerarlo ni modificar sus cotas. El nuevo plano técnico mide 2172 × 724 px.
- El nombre versionado del archivo invalida la miniatura optimizada anterior de Next.js sin debilitar la configuración de imágenes locales. El carrusel, el segundo slide, el poster de video y el resto de la página se mantienen sin cambios.
- Verificación: el SHA-256 del activo publicado y del archivo entregado coincide (`EDA82E599DF74777CF8F1A69C014EDF15D25DB334545B26251FE9FDB3B29A6DB`); el recurso local responde con el mismo contenido y `/proyectos` expone el hero actualizado con su texto alternativo descriptivo.
- Validaciones: `npx.cmd tsc --noEmit` OK. `npm.cmd run build` con clave pública de prueba de Turnstile aplicada solo al proceso completó 148 rutas. Persisten los avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-14 — Renovación fotográfica de casos de Proyectos

- Se inventariaron las diez fotografías de `C:\Users\cvillagran\Desktop\Fotos editar`. Se descartaron de la galería comercial las tomas de obra exterior inconclusa y la imagen con corrupción visual; se seleccionaron cinco instalaciones interiores representativas.
- Se generaron correcciones documentales no destructivas mediante la herramienta de edición de imágenes: balance de blancos, exposición, contraste local, nitidez y reducción de ruido. Las instrucciones preservaron mobiliario, instalaciones, geometría, personas, equipos, textos y logotipos reales, sin agregar ni retirar elementos.
- Se reemplazaron las cinco imágenes del reel de casos por `gallery-laboratorio-modular.png`, `gallery-mesones-almacenamiento.png`, `gallery-redes-gases.png`, `gallery-laboratorio-operativo.png` y `gallery-laboratorio-docente.png`.
- Ante la falta de una relación verificable entre las nuevas fotografías y los nombres de cliente previos, se reemplazaron esas atribuciones por categorías, títulos, ubicaciones y textos alternativos técnicos genéricos. No se afirma que una foto corresponda a SEREMI, CMP, Carabineros, PUC o Bureau Veritas.
- Verificación local en `/proyectos#evidencia-ejecucion`: los cinco botones de ampliación y sus imágenes/textos alternativos se encuentran presentes. `npx.cmd tsc --noEmit` OK. `npm.cmd run build` con clave pública de prueba de Turnstile aplicada solo al proceso completó 148 rutas. Persisten los avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-14 — Ampliación de galería de Proyectos

- Se revisó nuevamente la carpeta `C:\Users\cvillagran\Desktop\Fotos editar`: contiene nueve fotografías válidas y un archivo generado con corrupción visual. La toma usada inicialmente para "Laboratorio operativo equipado" (`PFIZER 048.JPG`) se reemplazó por otra instalación interior disponible en la carpeta.
- Se añadió una segunda evidencia de infraestructura exterior terminada. Ambas fotografías se corrigieron de forma documental (exposición, balance de blancos y contraste) sin agregar, retirar ni alterar instalaciones, equipos, personas o materiales; para la toma exterior se aplicó un recorte de encuadre que excluye sacos en primer plano.
- Se publicaron `gallery-laboratorio-operativo-20260914.png` y `gallery-modulo-tecnico-exterior.png`. La galería ahora tiene seis casos. Los títulos se mantienen genéricos y descriptivos porque no existe evidencia que permita atribuir estas imágenes a un cliente concreto.
- No se incorporaron `Firefly Generated Image.jpg` ni `IMG_20220316_133637.jpg` por corrupción visual, ni `1711377564329.jpg` por mostrar una estructura exterior incompleta. Se conservaron los archivos fuente sin modificaciones.
- Verificación local: `/proyectos#evidencia-ejecucion` expone seis controles de ampliación, incluidos "Laboratorio operativo equipado" y "Módulo técnico exterior". Ambos recursos se cargaron correctamente mediante `next/image` (`naturalWidth: 332`). `npx.cmd tsc --noEmit` finalizó correctamente. El primer intento de `npm.cmd run build` fue bloqueado por el sandbox con `spawn EPERM`; al reintentar fuera del sandbox con una clave pública de prueba de Turnstile solo para el proceso, el build completó 148 rutas. Persisten los avisos existentes de Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-15 — Corrección Hanon D50/D200 y auditoría de PDFs/modelos

- Se ejecutó el protocolo del repositorio: lectura de `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `CLAUDE.md`, revisión del último commit `f4217d9` y de esta bitácora. `sync-check.sh codex` se intentó mediante Git Bash y fue bloqueado por el entorno con `couldn't create signal pipe, Win32 error 5`.
- Se inventariaron y revisaron los cinco archivos de la carpeta fuente Hanon para D50/D200. El PDF fuente `Ficha tecnica del producto.pdf` existe; el archivo publicado `public/productos/hanon-d50-d200/hanon-d50-d200-ficha-tecnica.pdf` ya era idéntico (SHA-256 `8378C3F6A6B1BFCAD506DB8EA19E1739A98064F891C7960491D2330AA355699B`). El fallo provenía exclusivamente del enlace genérico inexistente `brochure-d50-d200.pdf`.
- Se corrigió el enlace para servir el PDF publicado real y se incorporó una tabla de comparación de modelos: D200 usa disco automático de 120 posiciones (con variante de 40 posiciones exclusiva de D200) y D50 usa disco de 60 posiciones. Se declararon como compartidos únicamente el rango 0,1–500 mg N, RSD ≤0,5 %, dimensiones 735 × 560 × 560 mm y potencia 2000 W.
- Verificación local: la página renderiza el título de comparación y ambos muestreadores; el PDF responde HTTP 200, `application/pdf`, 2.035.435 bytes.
- Se recorrieron las 107 fichas publicadas y se verificaron 98 enlaces PDF renderizados contra `public`: 0 enlaces rotos y 0 errores de carga de página. El informe completo está en `docs/auditorias/auditoria-catalogo-pdfs-modelos-2026-09-15.md`; identifica ocho fichas de familia cuya diferenciación visible requiere fuente adicional antes de corregirse.
- Validaciones: `npx.cmd tsc --noEmit` OK. El primer `npm.cmd run build` fue bloqueado por el sandbox en `spawn EPERM` tras compilar; reintentado fuera del sandbox con clave pública de prueba de Turnstile aplicada solo al proceso, completó 148 rutas. Persisten los avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-15 — Diferenciación de familias Infitek MCA, USC-M, DON-H, LYO60B y FMH-P

- Se ejecutó el protocolo del repositorio: lectura de `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `CLAUDE.md`, revisión de los diez commits recientes, de esta bitácora y del estado limpio inicial. `sync-check.sh codex` se intentó mediante Git Bash y fue bloqueado por el entorno con `couldn't create signal pipe, Win32 error 5`.
- Se inventariaron y revisaron todos los archivos de las cinco carpetas fuente Infitek: descripciones, cinco PDFs técnicos e imágenes. Se verificaron los modelos MCA110-10/5/2/1A, los doce USC-M, DON-H45/70/140/230 y sus opciones E, LYO60B-1S/1P/1T/1PT, y FMH-P1200A/1500A/1800A.
- Se añadió un patrón reutilizable de tabla comparativa de modelos y se aplicó únicamente a las cinco fichas autorizadas. MCA se diferencia por legibilidad; USC-M por capacidad, transductores y presencia de válvula/manija; DON-H por capacidad y material interior E; LYO60B por configuración de cámara; y FMH-P por dimensiones, extracción y peso.
- La fuente primaria de LYO60B documenta condensador de 6,5 L y vacío sin carga ≤5 Pa. Esos valores se usan en la nota común de la tabla y no se repite el dato distinto de 9 L/<10 Pa recibido en el encargo.
- Se revisaron las dos carpetas Decent solicitadas. No contienen PDFs: Rodillo tiene texto que nombra DHT, pero la lámina técnica JPG nombra DBR; DVM tiene una lámina JPG sin PDF. DHT/DBR y DVM no se modificaron. DDO tampoco se tocó. El informe de auditoría se actualizó para dejar como pendientes solamente DHT/DBR, DVM y DDO, con el motivo de cada caso.
- Verificación local: `/productos/infitek-mca-series`, `/productos/infitek-usc-m-series`, `/productos/infitek-don-h-series`, `/productos/infitek-lyo60b-series` y `/productos/infitek-fmh-pa-series` respondieron HTTP 200 y expusieron el título de su tabla comparativa.
- Validaciones: `npx.cmd tsc --noEmit` OK. El primer `npm.cmd run build` no pudo descargar Inter y Manrope por la restricción de red; reintentado fuera del sandbox con clave pública de prueba de Turnstile aplicada solo al proceso, completó 148 rutas. Persisten los avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-15 — Previsualizador reutilizable de documentación PDF

- Se ejecutó el protocolo del repositorio: lectura de `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `CLAUDE.md`, revisión del último commit `7cb092d` y de esta bitácora. `sync-check.sh codex` se intentó mediante Git Bash y fue bloqueado por el entorno con `couldn't create signal pipe, Win32 error 5`.
- La auditoría confirmó que los 112 enlaces PDF del catálogo se resuelven centralmente en `src/app/productos/[slug]/page.tsx`, mediante `brochureHref` y `technicalSheetLinks`; `mock-products.ts` y `product-detail-tabs.tsx` no los renderizan. Se sustituyeron los dos puntos de salida (botón de documento único y lista de familia) por `PdfPreviewButton`, sin cambiar datos técnicos, textos ni URLs de las fichas.
- El nuevo componente abre un modal accesible en la página actual, incluye cierre por botón, fondo y tecla Escape, devuelve el foco al disparador, bloquea el desplazamiento de fondo y entrega una descarga directa desde el modal. Antes de crear el iframe, comprueba la respuesta HTTP; para 405/501 usa una petición de rango. Un error HTTP, red o tiempo de espera de 12 segundos muestra el mensaje claro de que no se pudo cargar la vista previa.
- La primera revisión visual reveló que los PDFs heredaban `X-Frame-Options: DENY` y `frame-ancestors 'none'`, bloqueando el iframe aun cuando el archivo existía. `next.config.ts` conserva `DENY` para las páginas y habilita exclusivamente `/productos/**/*.pdf` con `SAMEORIGIN`, `frame-src 'self'` y `frame-ancestors 'self'`. La comprobación de producción confirmó HTTP 200 y esos encabezados para el PDF, mientras la página mantiene `X-Frame-Options: DENY`.
- Verificación en navegador local: Hanon D50/D200 abre un documento individual; Thermo Element Series abre sus dos documentos por familia; Infitek MCA abre su folleto. Los tres exponen modal, iframe y botón Descargar. Una prueba Playwright aislada interceptó solo el PDF Hanon como 404 y confirmó el texto `No se pudo cargar la vista previa de este documento.` sin introducir enlaces rotos en datos publicados.
- Validaciones: `npx.cmd tsc --noEmit` OK. `npm.cmd run build` inicial fue bloqueado por `spawn EPERM` al iniciar TypeScript; reintentado fuera del sandbox con clave pública de prueba de Turnstile aplicada solo al proceso, completó las 148 rutas. Persisten los avisos existentes de `MODULE_TYPELESS_PACKAGE_JSON` y Upstash sin configurar. No se ejecutó `git reset`, `git clean` ni `git checkout`.

## 2026-09-15 — Visor PDF en ventana completa independiente

- Por solicitud, el visor dejó de usar un modal dentro de la ficha. Cada enlace de documentación abre `/documentos/vista-previa` en una pestaña o ventana nueva del navegador (`target="_blank"` con `noopener noreferrer`), donde la vista ocupa toda la pantalla disponible.
- Se creó la ruta dinámica `src/app/documentos/vista-previa/page.tsx` y el componente cliente `PdfPreviewPage`. Conservan el visor nativo mediante iframe, la descarga directa, el cierre de la pestaña y el acceso al catálogo.
- La ruta solo acepta rutas PDF publicadas bajo `/productos/`; rechaza traversal (`..`) y barras invertidas. La verificación HTTP previa conserva el fallback de petición de rango para servidores sin HEAD y muestra `No se pudo cargar la vista previa de este documento.` ante 404, red o tiempo de espera.
