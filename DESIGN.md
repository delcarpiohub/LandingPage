...
- **Acento decorativo por industria** (`industryAccent` en `solution-editorial-page.tsx`): un punto de color (con anillo/borde propio para no perder legibilidad con el amarillo) junto a cada técnica en la tabla y cada caso de aplicación. Nunca como color de texto corrido ni como color de acción — sigue habiendo un solo terracota interactivo en toda la página, sin excepción, por la "Regla de la Voz Única" ya documentada.
