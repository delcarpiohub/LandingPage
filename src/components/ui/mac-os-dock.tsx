...
// Icon components
const HomeIcon = ({ className = "w-4 h-4" }: IconProps) => (
...
// Separator component
const Separator = () => <div className="w-px h-8 bg-gray-300 dark:bg-gray-600 mx-2" />;
...
type NavItem = { href: string; icon: React.ComponentType<IconProps>; label: string };
type SocialItem = { name: string; url: string; icon: React.ComponentType<IconProps> };

// Data configuration
const DATA: { navbar: NavItem[]; contact: { social: Record<string, SocialItem> } } = {
  navbar: [
...
export function DockTheme() {
  ...
  return (
    <div className={`${isDark ? "dark" : ""} transition-colors duration-300`}>
      <div className="min-h-screen min-w-screen bg-gray-50 dark:bg-gray-900 flex flex-col items-center justify-center p-8">
        {/* Title */}
        <motion.h1
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="pointer-events-none text-center text-8xl font-semibold leading-none bg-gradient-to-b from-black to-gray-400 dark:from-white dark:to-gray-600 bg-clip-text text-transparent mb-12"
        >
          Animation
        </motion.h1>
        ...
        <Dock>
          ...
          {Object.entries(DATA.contact.social).map(([name, social]) => (
            <DockItem key={name} tooltip={name}>
              <a
                href={social.url}
                className="w-12 h-12 rounded-full bg-transparent hover:bg-gray-100 dark:hover:bg-gray-800 flex items-center justify-center transition-colors"
